<?php 
	...
	switch ($proses) {
		case 'add':
			try {
				mysqli_query ($koneksi, "INSERT INTO `data_layanan_jenis` (`id_kategori`, `sub_kategori`, `nama_jenis_layanan`, `harga`, `deskripsi_jenis_layanan`) VALUES ('$idKategori', 'Software', '$namaJenis', '$harga', '$deskripsi')") or die ($koneksi);
		...
		case 'edit':
			try {
				mysqli_query($koneksi, "UPDATE `data_layanan_jenis` SET `id_kategori` = '$idKategori', `sub_kategori` = 'Software', `nama_jenis_layanan` = '$namaJenis', `harga` = '$harga', `deskripsi_jenis_layanan` = '$deskripsi' WHERE `id_layanan_jenis` = '$id';") or die ($koneksi);
				$_SESSION['message-type'] = "success";
	...
	if ($proses == 'remove') {
		echo "<script>window.location.replace('?content=data_layanan_jenis_software');</script>";
	} else {
