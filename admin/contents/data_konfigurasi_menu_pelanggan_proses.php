<?php 
	...
	switch ($proses) {
		case 'add':
			try {
				mysqli_query ($koneksi, "
					INSERT INTO `data_konfigurasi_menu_pelanggan` (
						`nama`,
						`parent`,
						`url`,
						`konfigurasi_css`
		...
		case 'edit':
			try {
				mysqli_query($koneksi, "
					UPDATE `data_konfigurasi_menu_pelanggan`
					SET
						`nama` 				= '$nama_menu',
						`parent`			= '$parent',
						`url`				= '$url',
						`konfigurasi_css` 	= '$konfigurasi_css'
					WHERE `id_konfigurasi_menu_pelanggan`	= '$id';
				") or die ($koneksi);
				$_SESSION['message-type'] = "success";
		...
		case 'remove':
			try {
				mysqli_query($koneksi, "
					DELETE FROM `data_konfigurasi_menu_pelanggan`
					WHERE `id_konfigurasi_menu_pelanggan` = '$id'
				") or die ($koneksi);
				$_SESSION['message-type'] = "success";
	...
	echo "
		<script>
			location.replace('?content=data_konfigurasi_menu_pelanggan');
		</script>
	";
?>