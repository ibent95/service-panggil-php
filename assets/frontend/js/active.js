...
$(document).ready(function() {
    ...
    $('body').on('change load', 'input#datang_ke_lokasi', function() {
        if ($(this).is(":checked")) {
            $('div#form-lanjutan').show(250);
    ...
    if ($('input#datang_ke_lokasi').is(":checked")) {
        $('div#form-lanjutan').show(250);
        // akumulasiHargaPengantaran("+");
    }

    $('body').on('click', 'a#detail_teknisi', function(event) {
    ...
    $('div#modal_daftar_jenis_layanan').on('show.bs.modal', function(event) {
        var button = $(event.relatedTarget); // Button that triggered the modal
        var recipient = button.data('id'); // Extract info from data-* attributes
        var modal = $(this);
        modal.find('.modal-body a#button-order-layanan').attr('href', '?content=perbaikan_primary_form&action=tambah&id_kategori=' + recipient).val();
        $.ajax({
            type: "GET",
            url: "functions/function_responds.php?content=modal_jenis_layanan",
            data: {
                id: recipient,
                data: 'full'
            },
            ...
            success: function(response) {
                ...
                modal.find('.modal-body tbody#result').html(response);
            }
        });
